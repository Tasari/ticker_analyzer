...
- A separate Account Statement area imports eToro XLSX statements in memory without persisting the uploaded file. Its Analysis tab reports the period-end portfolio snapshot, cash-flow-adjusted P/L, ROI, annualized ROI, an explicitly estimated Modified Dietz TWR, a reconciled P/L waterfall, and holdings exposure. An inclusive date selector provides exact realized P/L, closed-position P/L, dividends, fees, other realized activity, external flows, and a cumulative chart for subperiods; total portfolio returns remain limited to the full statement because eToro does not include historical open-position valuations. Its Data preview tab retains the bounded worksheet viewer.
