...
- Estimate an assumption-driven Fair Value range independently of the Value score. Editable Bear/Base/Bull scenarios combine a simplified revenue-and-FCF-margin DCF, earnings multiple, FCF multiple, and dividend discount model; unavailable methods remain visible with an explicit data reason and do not block the other estimates.
- Simulate a portfolio from every analyzed ticker with 10,000 initial capital by default, equal or custom weights, a configurable cash allocation, fractional shares, periodic contributions, monthly/quarterly/annual rebalancing, and USD/EUR/PLN conversion. The Simulation tab compares buy-and-hold with rebalancing, supports reinvested or cash dividends, estimated commissions, spread and taxes, and reports nominal and inflation-adjusted results, TWR, CAGR and transaction friction. Risk analytics include Sharpe, Sortino, Calmar, downside deviation, historical daily VaR/Expected Shortfall, worst month/year, drawdown and recovery durations, volatility, and dividend-adjusted component correlations.
- Maintain a persistent Watchlist independent of Stock Analyzer selections. Each ticker can alert on rating changes, upper/lower price or score thresholds, newly available or missing data, and failed refreshes kept in a dedicated Check again queue without discarding the last successful snapshot.
...
- A precomputed exchange-based Large Cap Ranking covering 1,000 US-listed instruments (including foreign ADRs) plus up to 100 listings per supported XTB exchange, ordered by scoring-v5 Overall Score.
- Supported ranking venues are the US exchanges, Warsaw Stock Exchange, London Stock Exchange, Xetra, Euronext Paris/Lisbon/Amsterdam/Brussels, Bolsa de Madrid, Borsa Italiana, Vienna Stock Exchange, SIX Swiss Exchange, Nasdaq Copenhagen/Helsinki/Stockholm, and Oslo Bors. Issuer country is metadata and never an admission rule.
- One ranking update refreshes Stocks, ETFs, and Crypto, while one timestamped ZIP download contains every available ranking snapshot. Stock snapshots can be restored through the validated JSON import. Each completed stock refresh stores a quality report with market coverage, failure categories, rating distribution, and score/rank/rating changes from the previous snapshot.
...
Use **Download all rankings** before a hosted-container rebuild to save the stock, ETF, and crypto snapshots in one ZIP. The stock JSON can be restored from its import expander; imports are bounded, schema-validated, duplicate-checked, and atomically replace the active snapshot. In production mode, enable this explicitly with `ALLOW_RANKING_IMPORT=true`.

Before a full refresh, run a non-destructive exchange-based smoke build. It scans at most 20 US-listed instruments plus five each from the Warsaw Stock Exchange, London Stock Exchange, and Xetra, uses three workers, disables long analysis retries, and writes to the ignored `data/large_cap_ranking_smoke.json` file. Runtime and peak traced Python memory are included in its metadata:
